...
def distance_correlation(x, y):
    try:
        from dcor import distance_correlation as dcor_distance_correlation

        return dcor_distance_correlation(x, y)
    except RuntimeError:
        return _distance_correlation_numpy(x, y)


def _distance_correlation_numpy(x, y):
    x = np.asarray(x)
    y = np.asarray(y)
    if x.ndim == 1:
        x = x[:, None]
