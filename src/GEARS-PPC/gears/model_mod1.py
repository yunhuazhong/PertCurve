...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        # perturbation score embedding - transforms normalized pseudotime scores
        self.score_w = nn.Linear(1, hidden_size)
        self.score_lambda = 0.3  # Weight for score contribution
    ...
    def forward(self, data):
        ...
        # Extract perturbation scores from data
        ...
        if self.no_perturb:
            ...
            # Add perturbation score embeddings if available
            if pert_scores is not None:
                # Transform scores to embeddings: [batch_size, 1] -> [batch_size, hidden_size]
                score_emb = self.score_w(pert_scores.to(self.args['device']))
                # Expand to match gene dimensions: [batch_size, hidden_size] -> [batch_size * num_genes, hidden_size]
                score_emb = score_emb.repeat_interleave(self.num_genes, dim=0)
                # Add score embedding to base embeddings with weight
                base_emb = base_emb + self.score_lambda * score_emb
