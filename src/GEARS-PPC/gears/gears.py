...
class GEARS:
    ...
    def plot_perturbation(self, query, save_file = None, genes_to_plot = None, sort_genes = True):
        """
        ...
        genes_to_plot: list
            ordered list of genes to visualize
        sort_genes: bool
            whether to sort genes alphabetically before plotting, default False
        ...
        if genes_to_plot is not None:
            ...
            de_genes = [gene_raw2id[i] for i in adata.uns['top_non_dropout_de_20'][cond2name[query]]]
            if sort_genes:
                de_genes = sorted(de_genes)
            de_idx = [gene2idx[gene_raw2id[i]] for i in adata.uns['top_non_dropout_de_20'][cond2name[query]]]
            if sort_genes:
                gene_to_idx = {gene_raw2id[i]: gene2idx[gene_raw2id[i]] for i in adata.uns['top_non_dropout_de_20'][cond2name[query]]}
                de_idx = [gene_to_idx[g] for g in de_genes]
        ...
        if has_scores:
            query_mask = (adata.obs['condition'] == query).values
        ...
        # dpi 600
        fig, axes = plt.subplots(1, 2 if truth_subset is not None else 1, figsize=[18, 4.8], sharey=True, dpi=600)
        if not isinstance(axes, np.ndarray):
            axes = np.array([axes])

        def _plot_panel(ax, panel_truth, panel_title):
            ...
            for i in range(pred.shape[0]):
                ax.scatter(i + 1, pred[i], color='red')
        ...
        _plot_panel(axes[0], truth, f"{query} (all GT cells)")
        if truth_subset is not None:
            _plot_panel(axes[1], truth_subset, f"{query} (perturbation_score > 0.5)")
        
        fig.tight_layout()
        if save_file:
            # dpi 600
            fig.savefig(save_file, bbox_inches='tight', dpi=600)
        plt.show()
